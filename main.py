...
async def run_web_server():
    ...
    await site.start()
    print("Веб-сервер запущен: порт 8080")
        ...
        with open(LOG_FILE, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    ...
                    if datetime.strptime(data["time"], "%Y-%m-%d %H:%M:%S") > cutoff:
                        lines.append(line)
                except: continue
...
def analyze_growth(symbol):
    ...
    growth_days = sum(1 for i in range(len(closes)-1, 0, -1) if closes[i] > closes[i-1])
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    symbol, days = text[0], min(int(text[1]) if len(text)>1 and text[1].isdigit() else 5, 30)
    ...
    await update.message.reply_text(
        f"<b>{symbol}/USDT</b>\nЦена: <code>{price_fmt}</code>\n{emoji} За {days} {days_word}: <b>{growth:+.2f}%</b>",
        parse_mode='HTML'
    )
